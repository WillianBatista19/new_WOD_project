...
// fields at all, purely a static snapshot. Complements generateMagePdf's interactive version.
//
// Earlier versions of this function captured the WHOLE sheet as one tall image and sliced it
// by fixed page-height boundaries — that's what caused content to be cut mid-card, because the
// slice points have no idea where a card's edges are. Switching PDF libraries alone wouldn't
// have fixed that (jsPDF's addImage-with-negative-offset technique has the exact same flaw as
// the equivalent pdf-lib approach — same algorithm, different API). The actual fix is capturing
// and placing each card as its own unbroken image, only starting a new page when a card doesn't
// fit in the space remaining — a card is only ever sliced if it's individually taller than one
// whole page, which no section in this sheet currently is.
export async function generateMageFlatPdf(sheetElement: HTMLElement): Promise<Uint8Array> {
