...
// Outlined square + "/" mark for superficial damage — muted/gray, not red (only Aggravated is red).
const superficialCheckboxAppearanceProvider: AppearanceProviderFor<PDFCheckBox> = (_checkBox, widget) => {
  ...
  const on = [
    ...drawRectangle({ ...rectBase, color: COLORS.card, borderColor: COLORS.muted, borderWidth: 1 }),
    ...drawLine({ start: { x: inset, y: inset }, end: { x: width - inset, y: height - inset }, thickness: 1.3, color: COLORS.muted }),
  ]
  return { normal: { on, off } }
}

// Outlined square + "X" mark for aggravated damage — rendered in red.
...
function healthBlock(canvas: PdfCanvas, form: PDFForm, health: HealthBoxState[]) {
  ...
  health.forEach((state, index) => {
    ...
    const supName = reserveFieldName(`health_${index + 1}_sup`)
    const aggName = reserveFieldName(`health_${index + 1}_agg`)

    // Both widgets share the same rectangle — whichever is checked draws a fully opaque
    // box that visually covers the other's unchecked appearance underneath.
