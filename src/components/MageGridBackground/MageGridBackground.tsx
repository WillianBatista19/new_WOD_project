...
const MageGridBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        vec3 gridColor = vec3(0.3, 0.1, 0.8);
        ...
        color += vec3(0.6, 0.2, 1.0) * energy * line;
        float glow = smoothstep(0.1, 0.0, mouseDist);
        color += vec3(0.5, 0.2, 1.0) * glow * 0.5;
    ...
    const onResize = () => { const w = container.clientWidth, h = container.clientHeight; renderer.setSize(w, h); uniforms.iResolution.value.set(w, h); };
...
export default MageGridBackground;
