...
export const SmokeBackground: React.FC<SmokeBackgroundProps> = ({ smokeColor = "#808080" }) => {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => { renderer.render(now); animationFrameId = requestAnimationFrame(loop); };
    loop(0);
    return () => { window.removeEventListener('resize', handleResize); cancelAnimationFrame(animationFrameId); renderer.reset(); };
  }, []);

  useEffect(() => {
    const renderer = rendererRef.current;
    if (renderer) { const rgb = hexToRgb(smokeColor); if (rgb) renderer.updateColor(rgb); }
  }, [smokeColor]);
