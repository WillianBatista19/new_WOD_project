import { HEALTH_AGGRAVATED, HEALTH_EMPTY, HEALTH_SUPERFICIAL, HealthBoxState } from '../../types/vampire'
import { vampireHealthPenalty } from '../../data/vampireHealth'

interface VampireHealthTrackProps {
...
export default function VampireHealthTrack({ value, onChange }: VampireHealthTrackProps) {
  const handleClick = (index: number) => {
    onChange(value.map((state, i) => (i === index ? nextState(state) : state)))
  }

  const penalty = vampireHealthPenalty(value)
