...
const MistBackgroundLocal: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        vec2 q = vec2(fbm(uv + 0.07 * u_time), fbm(uv + vec2(1.0, 1.0)));
        vec2 r = vec2(fbm(uv + 1.0 * q + vec2(1.7, 9.2) + 0.15 * u_time), fbm(uv + 1.0 * q + vec2(8.3, 2.8) + 0.126 * u_time));
        float f = fbm(uv + r);
        vec3 baseColor = vec3(0.03, 0.03, 0.05);
        vec3 mistColor   = vec3(0.08, 0.09, 0.12);
        vec3 accentColor = vec3(0.15, 0.17, 0.22);
    ...
    let mouse = { x: 0, y: 0 };
    const handleMouseMove = (e: MouseEvent) => { mouse.x = e.clientX; mouse.y = window.innerHeight - e.clientY; };
    window.addEventListener('mousemove', handleMouseMove);
    let animationFrameId: number;
    const render = (time: number) => {
      if (canvas.width !== window.innerWidth || canvas.height !== window.innerHeight) { canvas.width = window.innerWidth; canvas.height = window.innerHeight; gl.viewport(0, 0, canvas.width, canvas.height); }
  ...
  return <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none" style={{ background: '#09090b' }} />;
};

export default MistBackgroundLocal;
