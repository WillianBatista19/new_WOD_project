...
export interface VampireQuickReferenceModalState {
  title: string
  topics: VampireReferenceTopic[]
}

export default function VampireQuickReferenceModal({
  state,
  onClose,
}: {
  state: VampireQuickReferenceModalState | null
  onClose: () => void
}) {
  useEffect(() => {
    if (!state) return
    ...
    <div className="lg:hidden fixed top-0 right-0 bottom-0 left-0 z-50 flex items-end sm:items-center justify-center">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          <VampireQuickReferenceContent topics={state.topics} />
        </div>
      </div>
    </div>
  )
